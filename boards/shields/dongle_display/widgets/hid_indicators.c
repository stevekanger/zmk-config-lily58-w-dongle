...
static void set_hid_indicators(lv_obj_t *label, struct hid_indicators_state state) {
    ...
    if (state.hid_indicators && state.hid_indicators == LED_CLCK) {
        strncat(text, "CAPS", 4);
    }
    // if (state.hid_indicators && state.hid_indicators == LED_NLCK) {
    //     strncat(text, "NUMS", 4);
    // }
    // if (state.hid_indicators && state.hid_indicators == LED_SLCK) {
    //     strncat(text, "SCROLL", 6);
    // }
...
static struct hid_indicators_state hid_indicators_get_state(const zmk_event_t *eh) {
    ...
    LOG_DBG("HID indicators raw: 0x%02X", ev->indicators);
